...
def check_decomposition(question,text,nlp):
    ...
    if "\n" in text:
        return ReturnError("Annotations must be in a single line")
    else:
        lines = text.split(";") # get each decomposition
        ...
        if lines[0].startswith("return"): # check if the first statement starts with return
            if "return" in lines[0][5:]:
                return DecompositionError("Each decomposition must start after a semicolon")
            ...
            for i in range(1,n): # check for all further lines
                if "return" in lines[i][6:]:
                    return ReturnError("Each decomposition must start after a semicolon")
